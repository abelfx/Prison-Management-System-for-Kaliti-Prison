...
const AddSecurityStaff = ({ isOpen, onClose, refreshList }) => {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!securityStaffData.phone) {
        ...
        validationErrors.phone =
          "Phone Number should start with 0 and be 10 digits";
      }
    ...
    if (step === 2) {
      ...
      if (securityStaffData.completedCourses.length === 0) {
        validationErrors.completedCourses =
          "At least one Completed Course is required";
      }
    }
    ...
    if (validateStep(step)) {
      try {
        const response = await axios.post(
          "https://prison-management-system-backend.onrender.com/api/securityStaff/add",
          securityStaffData
        );
        onClose();
        refreshList();
  ...
  return (
    <div
      className={`add-incident-overlay ${isOpen ? "inci-show" : "inci-hide"}`}
    >
      <div className=" add-incident-container">
        <FaWindowClose
          ...
          color="red"
          onClick={onClose}
          className="add-incident-close"
        />
        {step === 1 && (
          <PersonalInformation
            securityStaffData={securityStaffData}
        ...
        <div id="button-line">
          {step > 1 && (
            <button onClick={handlePrevious} className="add-incident-button">
              Previous
            </button>
          )}
          {step < 2 ? (
            <button onClick={handleNext} className="add-incident-button">
              Next
            </button>
          ) : (
            <button onClick={handleSubmit} className="add-incident-button">
              Submit
            </button>
          )}
        </div>
