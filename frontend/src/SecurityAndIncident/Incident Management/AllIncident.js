...
const AllIncident = () => {
  ...
  const getAllIncidents = async () => {
    try {
      const res = await axios.get(
        "https://prison-management-system-backend.onrender.com/api/incident/"
      );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/api/incident/delete/${deletingIncidentId}`
      );
      getAllIncidents();
          ...
          <div className="flex-container ">
            <div className="search-container">
              ...
              <div className="flex items-center justify-end gap-4">
                <button onClick={toggleAddDialog} className="add-button">
                  Add
                </button>
              </div>
            </div>
              ...
              <tbody className="">
                {filteredIncidents.map((incident) => (
                  <tr key={incident._id} className="">
                    ...
                    <td id="button-icons">
                      <button onClick={() => handleView(incident._id)}>
                        <IoEyeOutline color="green" size={20} />
                      </button>
                      <button onClick={() => handleUpdate(incident._id)}>
