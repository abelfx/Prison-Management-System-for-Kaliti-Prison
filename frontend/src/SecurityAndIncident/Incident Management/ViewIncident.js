  ...
  const getIncidentDetails = async (incidentId) => {
    try {
      const response = await axios.get(
        `https://prison-management-system-backend.onrender.com/api/incident/${incidentId}`
      );
          ...
          <div>
            <div style={{ padding: 4 }} ref={componentRef}>
              ...
              <div style={{ margin: "10px" }}>
                ...
                >
                  <p>
                    <strong>Reported By:</strong> {incidentDetails.reportedBy}
                  </p>
                  <p>
                    <strong>Date of Report:</strong> {dateOfReport}
                  </p>{" "}
                  {/* Display today's date */}
                </div>
                <p>
                  <strong>Report Number:</strong> {incidentDetails.reportNumber}
                </p>
                <div>
                  ...
                  >
                    ...
                    <p>
                      ...
                      {new Date(
                        incidentDetails.dateOfIncident
                      ).toLocaleDateString()}
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>Time of Incident:</strong> {incidentDetails.time}
                    </p>{" "}
                    {/* This seems to be incorrect. You might want to replace it with the actual time of the incident. */}
                  </div>
                </div>
                <p>
                ...
                <div>
                  ...
                  >
                    {incidentDetails.inmateNumber && (
                      <p>
                        <strong>Inmate Number:</strong>{" "}
                        {incidentDetails.inmateNumber}
                      </p>
                    )}
                    {incidentDetails.inmateName && (
                      <p>
