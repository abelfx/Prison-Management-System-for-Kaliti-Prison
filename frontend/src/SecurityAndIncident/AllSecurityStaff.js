...
const AllSecurityStaff = () => {
  ...
  const getAllSecurityStaff = async () => {
    try {
      const res = await axios.get(
        "https://prison-management-system-backend.onrender.com/api/securityStaff/"
      );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/api/securityStaff/delete/${deletingStaffId}`
      );
  ...
  const filteredSecurityStaff = securityStaffList.filter(
    (staff) =>
  ...
  return (
    <div className="security-home">
        ...
        <div className="main-content">
          ...
          <div className="flex-container">
            <div className="search-container">
              ...
              />
              <div>
                <button className="add-button" onClick={toggleAddDialog}>
                  Add
                </button>
              </div>
            </div>
              ...
              <tbody>
                {filteredSecurityStaff.map((staff) => (
                  <tr key={staff._id}>
                    ...
                    <td>{staff.nic}</td>
                    <td id="button-icons">
                      <button onClick={() => handleView(staff._id)}>
                        <IoEyeOutline color="green" size={20} />
                      </button>
                      <button onClick={() => handleUpdate(staff._id)}>
      ...
      <AddSecurityStaff
        ...
        refreshList={getAllSecurityStaff}
      />
      <ViewSecurityStaff
      ...
      <UpdateSecurityStaff
        ...
        refreshList={getAllSecurityStaff}
        securityStaffId={selectedStaffId}
      />
      <ConfirmationModal
