...
const VisitorDetails = ({ isOpen, onClose, refreshList, visitorId }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/api/visitor/delete/${visitorId}`
      );
            ...
            <div ref={componentRef} style={{ padding: "10px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div className="v-details">
                  <p>
                    <strong>First Name:</strong> {visitorDetails.firstName}
                  </p>
                  <p>
                    <strong>Last Name:</strong> {visitorDetails.lastName}
                  </p>
                  <p>
                    <strong>Date of Birth:</strong>{" "}
                    {formatDateOfBirth(visitorDetails.dateOfBirth)}
                  </p>
                  <p>
                    <strong>Gender:</strong> {visitorDetails.gender}
                  </p>
                  <p>
                    <strong>NIC:</strong> {visitorDetails.nic}
                  </p>
                  <p>
                    <strong>Email:</strong> {visitorDetails.email}
                  </p>
                  <p>
                ...
                <div className="v-details">
                  <p>
                    <strong>Inmate Number:</strong> {visitorDetails.inmateNo}
                  </p>
                  <p>
                    <strong>Inmate Name:</strong> {visitorDetails.inmateName}
                  </p>
                  <p>
                    <strong>Date of Visit:</strong>{" "}
                    {formatDateOfVisit(visitorDetails.dateOfVisit)}
                  </p>
                  <p>
                    <strong>Time of Visit:</strong> {visitorDetails.timeOfVisit}
                  </p>
                  <p>
