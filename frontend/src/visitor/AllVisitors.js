...
const AllVisitors = () => {
  ...
  const getAllVisitors = async () => {
    const res = await axios.get(
      "https://prison-management-system-backend.onrender.com/api/visitor"
    );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/api/visitor/delete/${deletingVisitorId}`
      );
      getAllVisitors();
        ...
        <div className="v-main-content">
          ...
          <div className="v-flex-container ">
            <div className="v-search-container">
              ...
              />
              <button onClick={toggleAddDialog} className="v-add-button">
                Add
              </button>
            </div>
          </div>
                ...
                {filteredVisitors.map((visitor) => (
                  <tr key={visitor._id}>
                    ...
                    <td id="v-button-icons">
                      <button onClick={() => handleView(visitor._id)}>
                        <IoEyeOutline color="green" size={20} />
                      </button>
                      <button onClick={() => handleUpdate(visitor._id)}>
                        <FaEdit color="blue" size={20} />
                      </button>
                      <button onClick={() => handleDelete(visitor._id)}>
                        <MdDeleteOutline color="red" size={20} />
                      </button>
                      <button onClick={() => handleCreate(visitor._id)}>
                        <FaCheck color="#3f6212" size={20} />
                      </button>
                    </td>
                  </tr>
                ))}
