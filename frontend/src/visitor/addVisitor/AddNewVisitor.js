...
const AddNewVisitor = ({ isOpen, onClose, refreshList }) => {
    ...
    if (step === 1) {
      ...
      if (!visitorData.firstName) {
        ...
        validationErrors.firstName =
          "First Name cannot contain symbols or numbers";
      }
      if (!visitorData.lastName) {
        validationErrors.lastName = "Last Name is required";
      } else if (!/^[a-zA-Z]+$/.test(visitorData.lastName)) {
        validationErrors.lastName =
          "Last Name cannot contain symbols or numbers";
      }
      ...
      if (!visitorData.nic) {
        ...
        if (!nicRegex.test(visitorData.nic)) {
          validationErrors.nic =
            "Invalid NIC format. Please enter exactly 16 digits.";
        }
      }
    ...
    if (validateStep(step)) {
      try {
        const response = await axios.post(
          "https://prison-management-system-backend.onrender.com/api/visitor/add",
          visitorData
        );
        onClose();
        refreshList();
  ...
  return (
    <div
      className={`v-add-incident-overlay ${
        isOpen ? "v-inci-show" : "v-inci-hide"
      }`}
    >
      <div className="v-add-incident-container">
        ...
        <div className="v-button-line">
          {step > 1 && (
            <button onClick={handlePrevious} className="v-add-incident-button">
              Previous
            </button>
          )}
          {step < 2 ? (
            <button onClick={handleNext} className="v-add-incident-button">
              Next
            </button>
          ) : (
            <button onClick={handleSubmit} className="v-add-incident-button">
              Submit
            </button>
          )}
        </div>
