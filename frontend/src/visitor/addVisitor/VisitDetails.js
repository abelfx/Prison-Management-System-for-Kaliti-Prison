...
const VisitDetails = ({ visitorData, handleChange, errors }) => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "https://prison-management-system-backend.onrender.com/api/visit"
      );
  ...
  const handleInmateNoChange = (event) => {
    ...
    const selectedInmate = inmateData.find(
      (inmate) => inmate.inmateNo === selectedInmateNo
    );
    if (selectedInmate) {
      setSelectedInmateName(selectedInmate.initialname);
      handleChange({ target: { name: "inmateNo", value: selectedInmateNo } });
      handleChange({
        target: { name: "inmateName", value: selectedInmate.initialname },
      });
    } else {
      ...
      <form>
        <div className="v-add-sec-div">
          ...
          <div>
            ...
            />
            {errors.inmateNo && (
              <span className="v-add-incident-eroor">{errors.inmateNo}</span>
            )}
          </div>
        </div>
        <div className="v-add-sec-div">
          ...
          <div>
            ...
            />
            {errors.inmateName && (
              <span className="v-add-incident-eroor">{errors.inmateName}</span>
            )}
          </div>
        </div>
        <div className="v-add-sec-div">
          ...
          <div>
            ...
            />
            {errors.dateOfVisit && (
              <span className="v-add-incident-eroor">{errors.dateOfVisit}</span>
            )}
          </div>
        </div>
        <div className="v-add-sec-div">
          ...
          <div>
            ...
            />
            {errors.timeOfVisit && (
              <span className="v-add-incident-eroor">{errors.timeOfVisit}</span>
            )}
          </div>
        </div>
        <div className="v-add-sec-div">
