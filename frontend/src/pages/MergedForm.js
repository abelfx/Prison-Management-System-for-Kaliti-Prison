...
function MergedForm() {
    ...
    if (validateForm()) {
      ...
      if (page === FormTitles.length - 1) {
        try {
          ...
          await axios.post(
            "https://prison-management-system-backend.onrender.com/inmate/addinmates",
            formDataToSend
          );
  ...
  const PageDisplay = () => {
    if (page === 0) {
      ...
      return (
        <AddCurrentAdmission formData={formData} setFormData={setFormData} />
      );
    }
  };
