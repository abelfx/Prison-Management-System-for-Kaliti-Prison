...
function AllHealthRecords() {
  ...
  const handleSearch = (event) => {
    ...
    const filtered = healthRecords.filter(
      (record) =>
        record.InmateName.toLowerCase().includes(
  ...
  const handleOkDeleteModal = async () => {
    try {
      const response = await axios.delete(
        `https://prison-management-system-backend.onrender.com/healthrecord/deletehealthrecords/${recordToDelete._id}`
      );
    ...
    <div>
      ...
      <div className="healthrecordContainer">
        ...
        <div className="healthflex">
          <p className="totalhealthrecords">
          ...
          <tbody>
            {filteredRecords.map((record) => (
              <tr key={record._id}>
                ...
                <td>
                  <button
                    className="HealthrecordView"
                    onClick={() => showViewModal(record)}
                  >
                    View
                  </button>
                  <button
                    className="HealthrecordUpdate"
                    onClick={() => showUpdateModal(record)}
                  >
                    Update
                  </button>
                  <button
                    className="HealthrecordDelete"
