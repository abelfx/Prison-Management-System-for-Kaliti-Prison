...
function CurrentInmate() {
  ...
  const [
    isVisibleHealthcareAppointmentModal,
    setIsVisibleHealthcareAppointmentModal,
  ] = useState(false);
  const [selectedInmateName, setSelectedInmateName] = useState("");
  const [selectedInmateForDeletion, setSelectedInmateForDeletion] =
    useState(null);
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      await axios.put(
        `https://prison-management-system-backend.onrender.com/inmate/${id}`,
        updatedData
      );
  ...
  const deleteModelHandler = async () => {
    try {
      const response = await axios.delete(
        `https://prison-management-system-backend.onrender.com/inmate/${selectedInmateForDeletion._id}`
      );
  ...
  return (
    <div className="container">
      ...
      <div>
        ...
        <Button type="primary" onClick={showAddModal} className="ButtonAdd">
          ADD
        </Button>
        <Modal
          ...
          footer={null}
          closeIcon={
            <span className="closeIcon" style={{ color: "red" }}>
              X
            </span>
          }
          width={700}
          ...
          {selectedInmate && (
            <div className="profileContainer">
              ...
              {selectedInmate.image ? (
                <img
                  className="image"
                  src={`https://prison-management-system-backend.onrender.com/uploads/${selectedInmate.image}`}
                  alt="Inmate Photo"
                  style={{
                    marginLeft: "230px",
                    width: "120px",
              ...
              <div className="profile-container">
                ...
                <div className="profile-item">
                  ...
                  <span className="profile-value">
                    :{" "}
                    {selectedInmate.birthday
                      ? new Date(selectedInmate.birthday)
                          .toISOString()
                          .split("T")[0]
                      : ""}
                  </span>
                ...
                <div className="profile-item">
                  ...
                  <span className="profile-value">
                    : {selectedInmate.emergencycontactname}
                  </span>
                </div>
                <div className="profile-item">
                  <strong className="profile-label">
                ...
                <div className="profile-item">
                  ...
                  <span className="profile-value">
                    :{" "}
                    {selectedInmate.admissionDate
                      ? new Date(selectedInmate.admissionDate)
                          .toISOString()
                          .split("T")[0]
                      : ""}
                  </span>
                ...
                <div className="profile-item">
                  ...
                  <span className="profile-value">
                    :{" "}
                    {selectedInmate.releaseDate
                      ? new Date(selectedInmate.releaseDate)
                          .toISOString()
                          .split("T")[0]
                      : ""}
                  </span>
                ...
                <div className="profile-item">
                  ...
                  <span className="profile-value">
                    : {selectedInmate.years} years, {selectedInmate.months}{" "}
                    months, {selectedInmate.days} days
                  </span>
                </div>
                <div className="profile-item">
                  <strong className="profile-label">Cell Number</strong>
                  <span className="profile-value">
                    : {selectedInmate.cellNumber}
                  </span>
                </div>
                <div className="profile-item">
                  <strong className="profile-label">Medical Conditions</strong>
                  <span className="profile-value">
                    : {selectedInmate.medicalConditions}
                  </span>
                </div>
                <div className="profile-item">
                  <strong className="profile-label">Additional Notes</strong>
                  <span className="profile-value">
                    : {selectedInmate.additionalNotes}
                  </span>
                </div>
                {selectedInmate.escapedDate !== null && (
                  <div className="profile-item">
                    ...
                    <span className="profile-value">
                      :{" "}
                      {
                        new Date(selectedInmate.escapedDate)
                          .toISOString()
                          .split("T")[0]
                      }
                    </span>
                ...
                {selectedInmate.foundDate !== null && (
                  <div className="profile-item">
                    ...
                    <span className="profile-value">
                      :{" "}
                      {
                        new Date(selectedInmate.foundDate)
                          .toISOString()
                          .split("T")[0]
                      }
                    </span>
              ...
              <div className="bottomContainer">
                <button
                  className="updateInmateProfile"
                  style={{ background: "#4682B4" }}
                  onClick={() => handleUpdateClick(selectedInmate)}
                >
                  UPDATE
                </button>

                <button
                  className="deleteInmateProfile"
                  onClick={() => showDeleteModal(selectedInmate)}
                >
                  DELETE
                </button>

                <button
                  className="wantedProfile"
                  type="primary"
                  onClick={() =>
                    handleOpenHealthcareAppointmentModal(
                      selectedInmate.fullname
                    )
                  }
                >
                ...
                <button
                  className="wantedProfile"
                  onClick={() => handleAddToWantedList(selectedInmate)}
                >
                  Add To Wanted List
                </button>

                <button
                  className="releasedProfile"
                  onClick={() => handleAddToReleasedList(selectedInmate)}
                >
                ...
                <button
                  ...
                  onClick={() => showPDFModal(selectedInmate)}
                >
                  Download Report
                </button>
                ...
                >
                  <InmateMedicalDetails
                    ...
                    onClose={() =>
                      setIsVisibleHealthcareAppointmentModal(false)
                    }
                  />
                </Modal>
        ...
        >
          {selectedInmate && (
            <CurrentInmatesPDFDocument selectedInmate={selectedInmate} />
          )}
        </Modal>
        ...
        <p className="sentence">
          (To view additional information about an inmate, simply click on any
          row in the table.)
        </p>
            ...
            {filteredInmates.map((inmate) => (
              <tr key={inmate._id}>
                <td onClick={() => showProfileModal(inmate)}>
                  {inmate.image ? (
                    <img
                      src={`https://prison-management-system-backend.onrender.com/uploads/${inmate.image}`}
                      alt="Inmate Photo"
                ...
                <td onClick={() => showProfileModal(inmate)}>
                  {inmate.birthday
                    ? new Date(inmate.birthday).toISOString().split("T")[0]
                    : ""}
                </td>
                <td onClick={() => showProfileModal(inmate)}>
                  {inmate.gender}
                ...
                <td className="buttonColumn">
                  <button
                    className="update"
                    onClick={() => handleUpdateClick(inmate)}
                  >
                    Update
                  </button>
                  <button
                    className="delete"
                    onClick={() => showDeleteModal(inmate)}
                  >
                    Delete
                  </button>
                </td>
