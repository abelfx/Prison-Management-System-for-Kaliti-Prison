...
const JailorList = () => {
  ...
  const [isVisibleJailorProfileModal, setIsVisibleJailorProfileModal] =
    useState(false);
  const [isVisibleJailorPDFModal, setIsVisibleJailorPDFModal] = useState(false);
  const [isVisibleDeleteJailorModal, setIsVisibleDeleteJailorModal] =
    useState(false);
  ...
  const updateJailor = async (id, updatedJailorData) => {
    try {
      await axios.put(
        `https://prison-management-system-backend.onrender.com/Jailors/${id}, updatedJailorData`
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/Jailors/${selectedJailor._id}`
      );
  ...
  return (
    <div>
      ...
      <div className="all-jailor">
        ...
        <Button type="primary" onClick={showAddModal} className="addbutton">
          Add
        </Button>
        <table>
          <thead>
          ...
          <tbody>
            {filteredJailors.map((jailor) => (
              <tr key={jailor._id}>
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {jailor.FirstName}
                </td>
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {jailor.LastName}
                ...
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {formatDate(jailor.DateofBirth)}
                </td>
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {jailor.NIC}
                </td>
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {jailor.ContactNumber}
                </td>
                <td onClick={() => showJailorProfileModal(jailor)}>
                  {jailor.Gender}
                </td>
                ...
                <td>
                  <Button
                    className="update-jailor-button"
                    type="primary"
                    onClick={() => handleUpdate(jailor)}
                  >
                    Update
                  </Button>
                </td>
                <td>
                  <Button
                    className="delete-jailor-button"
                    type="danger"
                    onClick={() => ShowJailorDeleteModel(jailor)}
                  >
                    Delete
                  </Button>
                </td>
        ...
        >
          {selectedJailor && (
            <UpdateJailor
              selectedJailor={selectedJailor}
              updateJailor={updateJailor}
              onUpdate={() => {}}
            />
          )}
        </Modal>
            ...
            <>
              <div>
                ...
                <p>
                  ...
                  {selectedJailor.EmergencyContactNumber}
                </p>
                ...
                <p>
                  Educational Background: {selectedJailor.EducationalBackground}
                </p>
                <p>
                  Relevant Certifications:{" "}
                  {selectedJailor.RelevantCertifications}
                </p>
                <p>
                  Training Courses Completed:{" "}
                  {selectedJailor.TrainingCoursesCompleted}
                </p>
                <p>Uniform Size: {selectedJailor.UniformSize}</p>
                <p>Issued Equipment: {selectedJailor.IssuedEquipment}</p>
                <p>
                  Equipment Training Status:{" "}
                  {selectedJailor.EquipmentTrainingStatus}
                </p>
                <p>Medical Conditions: {selectedJailor.MedicalConditions}</p>
                <p>Allergies: {selectedJailor.Allergies}</p>
                <p>
                  Emergency Medical Information:{" "}
                  {selectedJailor.EmergencyMedicalInformation}
                </p>
                <Button onClick={() => showPDFModel(selectedJailor)}>
                  Download PDF
                </Button>
              </div>
