    ...
    if (healthRecord) {
      form.setFieldsValue({
        ...
        dateOfBirth: healthRecord.dateOfBirth
          ? moment(healthRecord.dateOfBirth).subtract(1, "days")
          : null,
  ...
  const onFinish = async (values) => {
    try {
      ...
      const response = await axios.put(
        `https://prison-management-system-backend.onrender.com/healthrecord/updatehealthrecords/${healthRecord._id}`,
        values
      );
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
        <Form.Item
          ...
          rules={[
            ...
            {
              pattern: /^[a-zA-Z. ]+$/,
              message: 'Inmate Name must include only letters and "." symbol',
            },
          ]}
        >
          <Input />
            ...
            () => ({
              validator(_, value) {
                ...
                return Promise.reject(
                  new Error("Date of Birth must be at least 18 years ago")
                );
              },
            }),
          ]}
