  ...
  const validateDateOfBirth = (value) => {
    ...
    if (dob > today || dob <= hundredYearsAgo) {
      setErrors({
        ...errors,
        dateOfBirth:
          "Date of Birth must be at least 100 years ago and not in the future.",
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://prison-management-system-backend.onrender.com/healthrecord/addhealthrecords",
        formData
      );
    ...
    <div className="healthRecordFormContainer">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="formGroup">
          ...
          />
          {errors.medications && (
            <span className="error">{errors.medications}</span>
          )}
        </div>
        <div className="formGroup">
