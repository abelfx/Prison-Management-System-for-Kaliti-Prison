...
const DoctorList = () => {
  ...
  const [isVisibleDoctorProfileModal, setIsVisibleDoctorProfileModal] =
    useState(false);
  const [isVisibleDoctorPDFModal, setIsVisibleDoctorPDFModal] = useState(false);
  const [isVisibleDeleteDoctorModal, setIsVisibleDeleteDoctorModal] =
    useState(false);
  ...
  const updateDoctor = async (id, updatedDoctorData) => {
    try {
      await axios.put(
        `https://prison-management-system-backend.onrender.com/Doctors/${id}`,
        updatedDoctorData
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://prison-management-system-backend.onrender.com/${selectedDoctor._id}`
      );
        ...
        <table>
          ...
          <tbody>
            {filteredDoctors.map((doctor) => (
              <tr
                key={doctor._id}
                onClick={() => showDoctorProfileModal(doctor)}
              >
                ...
                <td>
                  <Button
                    type="danger"
                    onClick={() => ShowDoctorDeleteModel(doctor)}
        ...
        {selectedDoctor && (
          <>
            <div>
              ...
              <Button onClick={() => showPDFModel(selectedDoctor)}>
                Download PDF
              </Button>
            </div>
