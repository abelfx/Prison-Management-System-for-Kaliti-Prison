...
const CreateVisit = ({ isOpen, onClose, refreshList, visitorId }) => {
  ...
  useEffect(() => {
    if (visitorDetails && visitorDetails.timeOfVisit) {
      let duration = 0;
      if (checkoutTime) {
        const checkInTime = visitorDetails.timeOfVisit
          .split(":")
          .slice(0, 2)
          .join(":"); // Extracting hours and minutes
  ...
  const visitSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://prison-management-system-backend.onrender.com/api/visit/add",
        {
  ...
  return (
    <div
      className={`custom-modal-overlay ${
        isOpen ? "custom-sec-show" : "custom-sec-hide"
      }`}
    >
            ...
            >
              ...
              <div className="custom-flex-container">
                <div className="custom-info">
                  ...
                  <p>
                    {visitorDetails.firstName.charAt(0).toUpperCase() +
                      visitorDetails.firstName.slice(1)}{" "}
                    {visitorDetails.lastName.charAt(0).toUpperCase() +
                      visitorDetails.lastName.slice(1)}
                  </p>
                </div>
                <div className="custom-info">
